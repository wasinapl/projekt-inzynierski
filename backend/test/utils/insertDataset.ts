...
const tableToModelMap: { [key: string]: keyof PrismaClient } = {
    users: 'user',
    // Add other tables and their corresponding Prisma models here
}
...
export const insertDataset = async (table: string, dataset: string) => {
    ...
    if (!model) {
        throw new Error(`Model for table "${table}" is not defined in tableToModelMap.`)
    }

    const filePath = path.join(__dirname, '..', 'datasets', table, `${dataset}.json`)
    ...
    try {
        ...
        console.error(`Error inserting dataset "${dataset}" into "${table}":`, error)
        throw error
    }
}
